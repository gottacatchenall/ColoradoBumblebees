...
QUANTILE_COLORS = vcat([get(Makie.ColorSchemes.YlGn_5, i) for i in 0.15:0.15:1][begin:end-1]...)
years = ["2021-2040", "2041-2060", "2061-2080", "2081-2100"]
ssps = ["SSP126", "SSP245", "SSP370"]


function get_gained_interaction_map(sdms, bee, plant, ssp, year)
...
function get_gained_interaction_count(sdms, metaweb, ssp, year)
    ints = SpeciesInteractionNetworks.interactions(metaweb)

    gain_count = Float32.(similar(first(sdms)[2][:baseline][:range]))
    gain_count.grid .= 0
    for (bee, plant, _) in ints
        if bee ∈ keys(sdms) && plant ∈ keys(sdms)
            gain_count += get_gained_interaction_map(sdms, bee, plant, ssp, year)
        end
    end 
    return gain_count
end
...
gain_counts_by_ssp = Dict(
    [
        ssp=>[get_gained_interaction_count(sdms, metaweb, ssp, yr) for yr in years]
        for ssp in ssps
    ]
) 
...
qs = quantile(
    values(nodata(gain_counts_by_ssp["SSP370"][end], 0)),
    [0.2i for i in 1:4]
)
quantized_gain = []

for ssp in ssps
    nd = nodata(gain_counts_by_ssp[ssp][end], 0)
    ...
    push!(quantized_gain, quantized)
end 


quantized_gain[2] |> unique


function get_average_gained_interactions_per_elevation(
    gain, 
    elevation;
    num_bins = 50
)
    zerod_gain = nodata(gain, 0) 
    counts = zeros(Int, num_bins)

    interactions_vec = [zerod_gain[i] for i in eachindex(zerod_gain)]
    elevation_vec = [elevation[i] for i in eachindex(zerod_gain)]
...
begin 
    ...
    plot_quantized_loss!(g, (1,1), quantized_gain[1], title="SSP 1-2.6")
    plot_quantized_loss!(g, (1,2), quantized_gain[2], title="SSP 2-4.5")

    bottom_grid = GridLayout(g[2,:])
    plot_quantized_loss!(bottom_grid, (1,1), quantized_gain[3], title="SSP 3-7.0", halign=0.96)
    ...
    Colorbar(
        ...
        colorrange=(0.5,5.5),
        width=30,
        label = "Total Interactions Gained",
        flipaxis=false,
        labelsize = 16
    )
    ...
    ax = Axis(
        bottom_grid[1,3], 
        xlabel = "Average Gained Interactions", 
        ylabel="Elevation (m)",
        xgridvisible = false,
        ygridvisible= false,
    )
    ...
    for i in 3:-1:1
        elev_extrema, bin_centers, avg_gain = get_average_gained_interactions_per_elevation(
                gain_counts_by_ssp[ssps[i]][4], 
                elev;
                num_bins=50
        )
        x = avg_gain
        cutoff = findfirst(i -> i > x[end], x)
        barplot!(ax, bin_centers, x, direction=:x, label = SSP_LABELS[ssps[i]], color=(SCENARIO_COLORS[i]), gap=0)

    end 
    limits!(ax, 0, 70, 1000, 4200)
...
save(joinpath("plots", "interaction_gain.png"), f)
