...
function _fitmodel(ae::EnvironmentAutoencoder, data)
    ...
    models = [
        Chain(species_specific[s][:encoder], shared, species_specific[s][:decoder]) for
        s in species
    ]
    ...
    return species_specific, xs
end

function _makemodel(ae::EnvironmentAutoencoder, species, species_dfs)
    return _species_specific_models(ae, species, species_dfs), _shared_decoder(ae)
end
...
function _shrink(ae, env)
    ...
    return vcat(plantdf[shuffle(1:nrow(plantdf))[1:nrow(beedf)], :], beedf)
end
...
function _species_specific_encoder_decoder(ae, this_species_env_df)
    ...
    enc = Chain(
        ...
        [
            Dense(ae.species_encoder_dims[t], ae.species_encoder_dims[t + 1]) for
            t in 1:(length(ae.species_encoder_dims) - 1)
        ]...,
    )

    dec = Chain(
        [
            Dense(ae.species_decoder_dims[t], ae.species_decoder_dims[t + 1]) for
            t in 1:(length(ae.species_decoder_dims) - 1)
        ]...,
...
function _shared_decoder(ae::EnvironmentAutoencoder)
    return shared_dec = Chain(
        [
            Dense(ae.shared_decoder_dims[t], ae.shared_decoder_dims[t + 1]) for
            t in 1:(length(ae.shared_decoder_dims) - 1)
        ]...,
    )
end
