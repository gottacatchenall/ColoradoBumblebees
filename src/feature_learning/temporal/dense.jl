...
Base.@kwdef struct DenseAutoencoder{T} <: Temporal
    unit = Dense               # node unit for first layer
...
function _fitmodel(ae::DenseAutoencoder{Standard}, train, test)
...
function _fitmodel(ae::DenseAutoencoder{Variational}, train, test)
...
function _makemodel(ae::DenseAutoencoder{Standard})
...
function _makemodel(ae::DenseAutoencoder{Variational})
    enc_layer_sizes, dec_layer_sizes = ae.encoder_dims, ae.decoder_dims
