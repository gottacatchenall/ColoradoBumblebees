...
function load_interaction_data()
...
function _cut_species!(df)
    occ_df = load_occurrence_data()

    beeseq = CSV.read(datadir("public", "phylogeny", "raw_sequences", "bee_sequences.csv"), DataFrame).Bee
    plantseq = CSV.read(datadir("public", "phylogeny", "raw_sequences", "plant_sequences.csv"), DataFrame).Plants

    all_seq_sp = vcat(beeseq, plantseq)


    # adj this to only include if in both phylo and occurrence data   

    occ_species = unique(occ_df.species)
    filter!(x -> x.plant != "Hypochaeris radicata", df)
    filter!(x -> x.plant ∈ occ_species, df)
    filter!(x -> x.plant ∈ all_seq_sp, df)
    filter!(x -> x.pollinator ∈ occ_species, df)
